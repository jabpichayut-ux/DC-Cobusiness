...
function WarehousePromos() {
  ...
  useEffect(() => {
    fetch("/api/liff/promotions?type=warehouse").then(r => r.json()).then(d => setPromos(d.data || []));
  }, []);
  return (
    ...
    <div className="space-y-3">
      ...
      {spaces.map(s => (
        <div key={s.size} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${s.available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="flex border-b border-gray-200 bg-white sticky top-0 z-10">
        {[["home", "บริการ"], ["spaces", "พื้นที่ว่าง"], ["promo", "โปรโมชั่น"]].map(([t, label]) => (
          <a key={t} href={`?tab=${t}`}
            className={`flex-1 text-center py-3 text-sm font-medium ${tab === t ? "border-b-2 border-orange-600 text-orange-700" : "text-gray-500"}`}>
            {label}
          </a>
        ))}
      </div>
