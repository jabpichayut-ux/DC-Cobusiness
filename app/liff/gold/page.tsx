...
function GoldPromos() {
  ...
  useEffect(() => {
    fetch("/api/liff/promotions?type=gold").then(r => r.json()).then(d => setPromos(d.data || []));
  }, []);
  return (
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="flex border-b border-gray-200 bg-white sticky top-0 z-10">
        {[["home", "ราคาทอง"], ["promo", "โปรโมชั่น"]].map(([t, label]) => (
          <a key={t} href={`?tab=${t}`}
            className={`flex-1 text-center py-3 text-sm font-medium ${tab === t ? "border-b-2 border-yellow-500 text-yellow-600" : "text-gray-500"}`}>
            {label}
          </a>
        ))}
      </div>
