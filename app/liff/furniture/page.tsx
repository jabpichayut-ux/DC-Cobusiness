...
function FurniturePromos() {
  ...
  useEffect(() => { fetch("/api/liff/promotions?type=furniture").then(r => r.json()).then(d => setPromos(d.data || [])); }, []);
  return (
...
function FurnitureContent() {
  ...
  if (isLoading) return <div className="min-h-screen bg-teal-50 flex items-center justify-center"><div className="text-4xl animate-pulse">🪑</div></div>;
  if (error) return <div className="min-h-screen bg-teal-50 flex items-center justify-center"><p className="text-red-500">{error}</p></div>;
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="flex border-b border-gray-200 bg-white sticky top-0 z-10">
        {[["home", "สินค้า"], ["promo", "โปรโมชั่น"], ["catalog", "แคตตาล็อก"]].map(([t, label]) => (
          <a key={t} href={`?tab=${t}`} className={`flex-1 text-center py-3 text-sm font-medium ${tab === t ? "border-b-2 border-teal-600 text-teal-700" : "text-gray-500"}`}>{label}</a>
        ))}
      </div>
...
export default function FurniturePage() {
  return (
    <LiffProvider liffId={LIFF_ID}>
      <Suspense><FurnitureContent /></Suspense>
    </LiffProvider>
  );
}
