...
export default function UsersPage() {
  ...
  async function handleUpdate(id: string, tags: UserTag[], apartment_unit: string) {
    ...
    const json = await res.json();
    if (json.success) setUsers((prev) => prev.map((u) => (u.id === id ? json.data : u)));
  }

  const filtered = users.filter(
    (u) => !search || u.display_name?.toLowerCase().includes(search.toLowerCase()) || u.line_user_id.includes(search) || u.apartment_unit?.includes(search)
  );
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={loadUsers} className="bg-brand-600 text-white px-4 py-2 rounded-xl text-sm font-medium">รีเฟรช</button>
      </div>
      <div className="bg-white rounded-2xl shadow-sm p-4 mb-4">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ค้นหาชื่อ, LINE ID, เลขห้อง..." className="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
      </div>
