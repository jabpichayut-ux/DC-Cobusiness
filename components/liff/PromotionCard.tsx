    ...
    <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden ${isExpired ? "opacity-60" : ""}`}>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <span
            className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0 ${BUSINESS_COLORS[promotion.business_type]}`}
          >
            {BUSINESS_LABELS[promotion.business_type]}
          </span>
        </div>

        {promotion.description && (
          <p className="text-gray-600 text-xs leading-relaxed mb-3">{promotion.description}</p>
        )}
        ...
        {(promotion.starts_at || promotion.ends_at) && (
          <div className="text-xs text-gray-400">
            {promotion.starts_at && (
              <span>
                เริ่ม{" "}
                {new Date(promotion.starts_at).toLocaleDateString("th-TH", {
                  day: "numeric",
                  month: "short",
                })}
              </span>
            ...
            {promotion.ends_at && (
              <span>
                ถึง{" "}
                {new Date(promotion.ends_at).toLocaleDateString("th-TH", {
                  day: "numeric",
                  month: "short",
                  year: "2-digit",
                })}
