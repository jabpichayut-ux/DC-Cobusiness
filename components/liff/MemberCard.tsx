...
interface MemberCardProps { user: User; }
  ...
  return (
    <div className="bg-gradient-to-br from-brand-600 to-brand-700 rounded-2xl p-6 text-white shadow-xl">
      <div className="flex items-center gap-4 mb-4">
        {user.picture_url ? (
          <Image src={user.picture_url} alt={user.display_name || "Member"} width={64} height={64} className="rounded-full border-2 border-white/50" />
        ) : (
        ...
        <div>
          ...
          {user.apartment_unit && <p className="text-white/80 text-sm">ห้อง {user.apartment_unit}</p>}
        </div>
      </div>
      <div className="bg-white/10 rounded-xl p-3">
        ...
        <div className="flex flex-wrap gap-2">
          {user.tags.length > 0 ? (
            user.tags.map((tag) => <span key={tag} className="bg-white/20 text-white text-xs px-3 py-1 rounded-full">{TAG_LABELS[tag] || tag}</span>)
          ) : (
      ...
      <div className="mt-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-white text-sm font-medium">{new Date(user.created_at).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })}</p>
        </div>
...
export function TagBadge({ tag }: { tag: string }) {
  return <span className={`text-xs px-2 py-1 rounded-full font-medium ${TAG_COLORS[tag] || "bg-gray-100 text-gray-800"}`}>{TAG_LABELS[tag] || tag}</span>;
}
