...
const LiffContext = createContext<LiffContextType>({ profile: null, isLoading: true, error: null, isInClient: false });
...
export function LiffProvider({ liffId, children }: LiffProviderProps) {
  ...
  useEffect(() => {
    if (!liffId) { setError("LIFF ID ไม่ได้ตั้งค่า"); setIsLoading(false); return; }
    ...
    script.onerror = () => { setError("ไม่สามารถโหลด LINE SDK ได้"); setIsLoading(false); };
    document.head.appendChild(script);
  }, [liffId]);

  return <LiffContext.Provider value={{ profile, isLoading, error, isInClient }}>{children}</LiffContext.Provider>;
}
