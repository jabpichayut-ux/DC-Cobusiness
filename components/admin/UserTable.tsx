        ...
        <tbody className="divide-y divide-gray-100">
          {users.map((user) => (
            <tr key={user.id} className="hover:bg-gray-50">
              ...
              <td className="py-3 pr-4">
                {editingId === user.id ? (
                  <div className="flex flex-wrap gap-1">
                    {ALL_TAGS.map((tag) => (
                      <button key={tag} onClick={() => setEditTags((prev) => prev.includes(tag) ? prev.filter((t) => t !== tag) : [...prev, tag])} className={`text-xs px-2 py-0.5 rounded-full border ${editTags.includes(tag) ? TAG_COLORS[tag] + " border-transparent" : "border-gray-200 text-gray-400"}`}>{TAG_LABELS[tag]}</button>
                    ))}
                  </div>
                ) : (
                  <div className="flex flex-wrap gap-1">
                    {user.tags.map((tag) => <span key={tag} className={`text-xs px-2 py-0.5 rounded-full ${TAG_COLORS[tag]}`}>{TAG_LABELS[tag]}</span>)}
              ...
              <td className="py-3 pr-4">
                {editingId === user.id ? (
                  <input value={editUnit} onChange={(e) => setEditUnit(e.target.value)} placeholder="เลขห้อง" className="border rounded px-2 py-1 text-xs w-20" />
                ) : (
                  <span className="text-gray-700">{user.apartment_unit || "-"}</span>
                )}
              </td>
              <td className="py-3 pr-4 text-gray-500 text-xs">{new Date(user.created_at).toLocaleDateString("th-TH")}</td>
              <td className="py-3">
                {editingId === user.id ? (
                  <div className="flex gap-2">
                    <button onClick={() => saveEdit(user.id)} disabled={saving} className="text-xs bg-brand-600 text-white px-3 py-1 rounded-lg disabled:opacity-50">บันทึก</button>
                    <button onClick={() => setEditingId(null)} className="text-xs text-gray-500 px-3 py-1 rounded-lg">ยกเลิก</button>
                  </div>
                ) : (
                  <button onClick={() => startEdit(user)} className="text-xs text-brand-600 hover:underline">แก้ไข</button>
                )}
              </td>
            </tr>
          ))}
